...
from app.agent.templates.solution_review_templates import (
    SOLUTION_REVIEW_PERSONA,
...
class SolutionReviewNode:
    ...
    async def __call__(self, state: ArchitectState) -> dict:
        ...
        prompt = SOLUTION_REVIEW_PROMPT.format(
            requirement=requirement,
            solution=json.dumps(solution.model_dump() if solution else {}, indent=2),
        )
        llm_requests.labels(node="solution_review").inc()
        result: SolutionReviewOut = await self._llm.ainvoke(
            [SystemMessage(content=SOLUTION_REVIEW_PERSONA), HumanMessage(content=prompt)]
        )
